...
describe('TenantThemeService', () => {
  ...
  describe('applyBranding()', () => {
    ...
    it('should set --tp-bg CSS var with surfaceColor value', () => {
      ...
      expect(setPropertySpy).toHaveBeenCalledWith('--tp-bg', '#edebe0');
    });

    it('should set --tp-surface and --nm-bg CSS vars with surfaceColor value', () => {
      service.applyBranding(mockBranding);

      expect(setPropertySpy).toHaveBeenCalledWith('--tp-surface', '#edebe0');
      expect(setPropertySpy).toHaveBeenCalledWith('--nm-bg', '#edebe0');
    });
    ...
    it('should set --nm-shadow-dark and --nm-shadow-light CSS vars', () => {
      ...
      expect(setPropertySpy).toHaveBeenCalledWith('--nm-shadow-light', '#ffffff');
    });
    ...
    it('should set --tp-primary-light CSS var with secondaryColor value', () => {
      service.applyBranding(mockBranding);

      expect(setPropertySpy).toHaveBeenCalledWith('--tp-primary-light', '#b9a779');
      expect(setPropertySpy).toHaveBeenCalledWith('--tp-border', '#b9a779');
    });
    ...
    it('should set neumorphic accent RGB vars from primary color', () => {
      service.applyBranding(mockBranding);

      expect(setPropertySpy).toHaveBeenCalledWith('--nm-accent', '#428177');
    ...
    it('should set font-family on document.body', () => {
      service.applyBranding(mockBranding);

      // Browsers normalize single quotes to double quotes in style.fontFamily
      expect(document.body.style.fontFamily).toContain('Gotham Rounded');
      expect(document.body.style.fontFamily).toContain('sans-serif');
    });
  });
  ...
  describe('CSS var integration (actual DOM)', () => {
    it('should have correct computed CSS var values after applyBranding', () => {
      ...
      expect(root.style.getPropertyValue('--tp-bg')).toBe('#edebe0');
      expect(root.style.getPropertyValue('--nm-shadow-dark')).toBe('#988561');
      expect(root.style.getPropertyValue('--nm-shadow-light')).toBe('#ffffff');
