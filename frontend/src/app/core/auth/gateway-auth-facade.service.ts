...
export class GatewayAuthFacadeService implements AuthFacade {
  ...
  private readonly authUiText = inject(AuthUiTextService);
  ...
  login(credentials: LoginCredentials): Observable<LoginResponse> {
    ...
    if (!tenantSet) {
      return of({
        success: false,
        message: this.authUiText.text('AUTH-C-006'),
      });
    }
  ...
  logoutLocal(redirectReason: 'logged_out' | 'session_expired' = 'logged_out'): void {
    ...
    this.messageState.set(
      redirectReason === 'session_expired'
        ? this.authUiText.text('AUTH-I-032')
        : this.authUiText.text('AUTH-I-031'),
    );
