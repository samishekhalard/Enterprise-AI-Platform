...
describe('MasterDefinitionsSectionComponent', () => {
  ...
  const apiStub = {
    ...
    getObjectType: vi.fn((id: string) => of(MOCK_OBJECT_TYPES.find((t) => t.id === id))),
    createObjectType: vi.fn(),
    addAttributeToObjectType: vi.fn(() => of({})),
    addConnectionToObjectType: vi.fn(() => of({})),
    removeAttributeFromObjectType: vi.fn(() => of({})),
    removeConnectionFromObjectType: vi.fn(() => of({})),
    duplicateObjectType: vi.fn(() => of({})),
    restoreObjectType: vi.fn(() => of({})),
  ...
  it('addDraftConnection adds a connection when target and active name are valid', () => {
    ...
    expect(connections[0].targetObjectTypeId).toBe('ot-1');
    expect(connections[0].targetObjectTypeName).toBe('Server');
  });
