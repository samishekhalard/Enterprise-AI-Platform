...
async function navigateToDefinitions(
  ...
  // Object types mock — distinguish list vs individual GET
  await page.route('**/api/v1/definitions/object-types**', (route) => {
    if (route.request().method() === 'GET') {
      const url = route.request().url();
      // Individual object type GET: /object-types/{id} (not a list query)
      const individualMatch = url.match(/\/object-types\/([^/?]+)(?:\?|$)/);
      if (individualMatch) {
        const id = individualMatch[1];
        const item = pagedResponse.content.find((t: Record<string, unknown>) => t.id === id);
        if (item) {
