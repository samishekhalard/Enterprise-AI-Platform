...
test.describe('Quality Gates', () => {
  ...
  test('compatibility smoke for tenant manager tabs', async ({ page }) => {
    ...
    // Tenant manager shows a stats bar and tenant table
    await expect(page.locator('[data-testid="tenant-manager"]')).toBeVisible({ timeout: 8_000 });
    await expect(page.locator('[data-testid="stats-bar"]')).toBeVisible();

    // Click a tenant row to open the fact sheet with tabs
    const firstTenantLink = page.locator('.tenant-name-link').first();
    if (await firstTenantLink.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await firstTenantLink.click();
      await expect(page.locator('[data-testid="factsheet-tab-overview"]')).toBeVisible();
      await expect(page.locator('[data-testid="factsheet-tab-users"]')).toBeVisible();
      await expect(page.locator('[data-testid="factsheet-tab-branding"]')).toBeVisible();
      await expect(page.locator('[data-testid="factsheet-tab-license"]')).toBeVisible();
      await expect(page.locator('[data-testid="factsheet-tab-auth"]')).toBeVisible();
    }
  });
  ...
  test('visual regression baseline for tenant-manager stats', async ({ page }) => {
    await mockTenantManagerApis(page);
    await seedAuthSession(page);

    await page.goto('/administration?section=tenant-manager');
    const statsBar = page.locator('[data-testid="stats-bar"]');
    await expect(statsBar).toBeVisible({ timeout: 8_000 });

    // Verify stats content renders correctly
