...
# Bootstrap ai_db extensions as postgres. svc_ai intentionally does not have
# database-level CREATE or superuser privileges, so Flyway cannot install them.
...
# ============================================================================
echo ""
echo "=== Step 6: Transfer master_db table ownership to svc_tenant ==="
echo "(Flyway V4+ runs as svc_tenant and needs DDL privileges: ALTER TABLE, CREATE TRIGGER, etc.)"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "master_db" <<-EOSQL
  -- Transfer ownership so Flyway (running as svc_tenant) can run DDL migrations
  DO \$\$
  DECLARE
    r RECORD;
  BEGIN
    ...
    END LOOP;
  END \$\$;
  -- Also grant on any future objects
  ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON TABLES TO svc_tenant;
  ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON SEQUENCES TO svc_tenant;
EOSQL
