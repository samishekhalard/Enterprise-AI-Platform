...
> **References:** [ADR-009](../adr/ADR-009-auth-facade-neo4j-architecture.md), [EMS Neo4j Database Schema](../data-models/neo4j-ems-db.md)

---

## 1. Overview

This document provides the Low-Level Design for the Auth Facade service, which implements a provider-agnostic Dynamic Identity Broker pattern. The design enables runtime identity provider configuration per tenant without service restarts.
...
|----------|----------|---------|
| Strategic ADR | [ADR-009](../adr/ADR-009-auth-facade-neo4j-architecture.md) | Architecture decision and rationale |
| Graph Schema | [neo4j-ems-db.md](../data-models/neo4j-ems-db.md) | Canonical EMS Neo4j database schema |
| HLD (Building Blocks) | [05-building-blocks.md](../arc42/05-building-blocks.md) | High-level service architecture |
| Security Concepts | [08-crosscutting.md](../arc42/08-crosscutting.md) | Authentication patterns |
...
|----------|--------|-----------|
| Primary Database | Neo4j (Graph) | Identity relationships are naturally graph-based |
...
The primary implementation using Neo4j graph database:
...
|------|---------------|---------|
| `Tenant` | id, domain, name, active | Tenant identity root |
| `Provider` | name, vendor, displayName, iconUrl | IdP vendor definition (shared) |
| `Protocol` | type, version | Auth protocol (OIDC, SAML, LDAP) |
| `Config` | id, tenantId, clientId, clientSecretEncrypted, discoveryUrl, enabled, priority | Tenant-provider configuration |
| `User` | id, email, tenantId, externalId | User identity |
| `Group` | id, name, tenantId | Role aggregation |
| `Role` | name, tenantId, systemRole | RBAC role with inheritance |
...
3. Governance gate: custom CSS audit report (`frontend/test-results/custom-css-audit.json` and `.md`).
...
| `Neo4jProviderResolver` | **Implemented** | `/provider/Neo4jProviderResolver.java` |
...
| `GraphRoleService` | **Implemented** | `/service/GraphRoleService.java` |
...
| Neo4j Entities (all) | **Implemented** | `/graph/entity/*.java` |
...
| ADR-009 | **Complete** | `/docs/adr/ADR-009-auth-facade-neo4j-architecture.md` |
