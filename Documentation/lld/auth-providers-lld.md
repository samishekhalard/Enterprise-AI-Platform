...
> **Related Decision:** [Architecture section 9.3.3](../Architecture/09-architecture-decisions.md#933-neo4j-backed-identity-broker-adr-009)
...
2. **Configuration-Driven**: [AS-IS] Provider selection via Neo4j graph configuration. [TARGET] Provider selection via tenant-service (PostgreSQL). Neo4j is removed from the auth domain.
...
5. **Tenant Isolation**: [AS-IS] Per-tenant provider configuration in graph. [TARGET] Per-tenant provider configuration in tenant-service (PostgreSQL).
...
flowchart TB
    subgraph auth_facade["Auth Facade Service"]
        ...
        subgraph graph["[AS-IS] Neo4j Graph Layer — [TARGET] migrates to tenant-service PostgreSQL"]
...
> [AS-IS] The following diagram shows the current auth flow via auth-facade and Neo4j.
> [TARGET] Auth endpoints migrate to api-gateway (edge). Provider config resolution migrates to tenant-service (PostgreSQL). Auth-facade is removed after migration.
...
> [AS-IS] This diagram shows the current flow via auth-facade resolving provider config from Neo4j.
> [TARGET] Auth login endpoints migrate to api-gateway. Provider config is resolved from tenant-service (PostgreSQL). Auth-facade and Neo4j are removed from this flow.
...
sequenceDiagram
    ...
    AF->>Neo4j: [AS-IS] Check/Create User in graph — [TARGET] tenant-service (PostgreSQL)
...
> [AS-IS] The following Cypher migration script initializes provider nodes in Neo4j.
> [TARGET] Provider seed data migrates to a Flyway SQL migration in tenant-service (PostgreSQL). Neo4j initialization scripts for auth data are retired.
...
[AS-IS] All sensitive configuration values are encrypted using Jasypt before storage in Neo4j. [TARGET] Encrypted storage migrates to tenant-service PostgreSQL columns.
...
| [AS-IS] Neo4j migration scripts / [TARGET] Flyway SQL migration in tenant-service | 2h | DBA |
...
1. [AS-IS] Execute Neo4j migration `V007__add_additional_providers.cypher`. [TARGET] Create equivalent Flyway SQL migration in tenant-service for provider seed data.
2. Add indexes for new provider-specific fields ([TARGET] PostgreSQL B-tree indexes in tenant-service)
