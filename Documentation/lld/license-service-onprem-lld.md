...
| Auth-facade Feign client | `backend/auth-facade/src/main/java/com/ems/auth/client/LicenseServiceClient.java` | [IMPLEMENTED] [AS-IS] -- `validateSeat()` only; no `getUserFeatures()` method. [TRANSITION] Auth-facade is removed after migration; seat validation call moves to api-gateway or tenant-service. |
| Auth-facade circuit breaker | `backend/auth-facade/src/main/java/com/ems/auth/service/SeatValidationService.java:32` | [IMPLEMENTED] [AS-IS] -- `@CircuitBreaker(name = "licenseService")`. [TRANSITION] Migrates out of auth-facade. |
| Master tenant bypass | `backend/auth-facade/src/main/java/com/ems/auth/service/AuthServiceImpl.java:48` | [IMPLEMENTED] [AS-IS] -- `!RealmResolver.isMasterTenant(tenantId)`. [TRANSITION] Logic migrates to api-gateway or tenant-service. |
...
|------------|-----------|----------|
| auth-facade | Inbound (Feign call) | REST -- `GET /api/v1/internal/seats/validate`. [TRANSITION] auth-facade is removed after migration; this integration point moves to api-gateway or tenant-service. Neo4j is not involved in seat validation (license-service uses PostgreSQL directly). |
