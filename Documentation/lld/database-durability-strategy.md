...
| Neo4j 5.x Community | 1 container | 1 database ([AS-IS] auth-facade graph + definition-service data. [TARGET] definition-service canonical object types ONLY -- auth graph nodes migrate to tenant-service PostgreSQL and Neo4j is removed from the auth domain) | `{env}_neo4j_data`, `{env}_neo4j_logs` |
...
- All Neo4j graph data ([AS-IS] auth-facade provider/tenant/user/role nodes. [TARGET] definition-service canonical object types only -- auth nodes migrate to PostgreSQL)
